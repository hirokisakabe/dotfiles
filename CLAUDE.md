...
実装に移るときは、同じ WezTerm ウィンドウ内で別タブに実装セッションを起動する。

```bash
# 議論セッション（main）から実装セッションを別タブで開始
cwtstart <タスクの説明>
```

- `cwtstart` は新しい WezTerm タブを作成し、そのタブで `cwt <タスク>` を実行する。
- 左側（main）の議論セッションは維持したまま、右側/別タブで実装を進める。
